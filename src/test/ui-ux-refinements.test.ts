  ...
  describe("3. Graph Entity & Timeline Coherence", () => {
    it("handles multi-hop graph synthesis nodes and edges correctly", () => {
      const synthesis: UnifiedInvestigationModel = {
        ...
        graph: {
          nodes: [
            { id: "url:https://example.com", type: "url", label: "https://example.com", sources: ["Scanner"], confidence: 1.0, freshness: "current" },
            { id: "domain:example.com", type: "domain", label: "example.com", sources: ["URL Normalizer"], confidence: 1.0, freshness: "current" },
            { id: "ip:93.184.216.34", type: "ip", label: "93.184.216.34", sources: ["DNS"], confidence: 0.95, freshness: "current" },
          ],
          edges: [
            { id: "e1", source: "url:https://example.com", target: "domain:example.com", type: "resolves_to", label: "hosts", sources: ["Scanner"], confidence: 1.0 },
            { id: "e2", source: "domain:example.com", target: "ip:93.184.216.34", type: "hosted_by", label: "A record", sources: ["DNS"], confidence: 0.95 },
          ],
        },
      };
